                  ...
                  <div class="collapse navbar-collapse" id="main-nav">
					...
					@else
						...
						<li class="dropdown">
							...
							<li class="apply_now"><a href="{{ url('/chairman/create-image') }}">Add Image</a></li>
						@endif
						@endif
 ...
 @if(isset($galleries))
              ...
              <div class="col-xs-6 col-sm-3">
                <div class="aboutImage">
                 <a href="{{ url('/single-gallery/'.$event->id) }}">
				 <?php $exam_env = (new \App\Classes\Data\AfrakenData())->GetAfrakenEnv();
		                                 $photo_url=null;
		                                  if(isset($event->photo)){
		                                    if ($exam_env === 'production') {
					                        $bucket=Config('filesystems.disks.s3.bucket');
					                        $key='uploads/files/photo/'.$event->photo->url;
                                            $s3 = Storage::disk('s3');
                                            $photo_url=$s3->getDriver()->getAdapter()->getClient()->getObjectUrl($bucket, $key);
				                            }else{
					                      $photo_url=asset('uploads/files/images/'.$event->photo->url);
				                          }
		                                  }?>
                    <img src="{{$photo_url}}" alt="" class="img-responsive" />
                    <div class="overlay">
                      <p>{{$event->title}}</p>
                    </div>
                    <span class="captionLink">{{$event->sub_committee->name}}<span></span></span>
                  </a>
                </div><!-- aboutImage -->
